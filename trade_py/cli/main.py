...
_HIDDEN_ALIASES: dict[str, tuple[str, str | None]] = {
    ...
    "doctor": (
        "status",
        "DeprecationWarning: 'trade doctor' is deprecated; use 'trade status' instead.",
    ),
    "inspect": (
        "__inspect_shim__",
        None,
    ),  # inspect.py handles per-subcommand warnings/forwarding
...
def main(argv: list[str] | None = None) -> int:
    ...
    canonical_domains = [
        ...
        (
            "run",
            "run",
            "统一触发入口 (DAG / daily 流水线 / agenda / job / belief / recommend / picks)",
        ),
        ...
        (
            "observatory",
            "observatory",
            "BTC Observatory — 目录/快照/研究运维 (research --commit 写入)",
        ),
    ...
    domain_lines = "\n".join(f"  {name:<10}  {desc}" for name, _mod, desc in canonical_domains)
    ...
    parser.add_argument(
        "domain",
        choices=[n for n, _m, _d in domains],
        metavar="<域>",
