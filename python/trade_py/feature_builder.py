...
and inference. Four feature groups follow the plan (Section 4.3):

  Group A – Event features:      event_type, magnitude, actor_risk_score, ...
  Group B – KG relationship:     kg_hop_distance, kg_edge_weight, ...
  Group C – Asset technical:     technical signals (mirror C++ TechnicalSignal)
  Group D – Market environment:  sector rank, northbound flow, ...
...
# Group C: Asset technical state (19 fields – mirror TechnicalSignal)
GROUP_C_COLS = [
    "kdj_zone",             # 0/1/2
    "kdj_cross",            # -1/0/+1
...
# Group D: Market environment (5 fields)
GROUP_D_COLS = [
    "market_trend_20d",     # MA20 return of benchmark, proxied by composite index
    "sector_rank_20d",      # sector's 20d return rank among all sectors [0,1]
    "northbound_5d_net",    # 5-day north-bound fund net flow (billion CNY)
    "margin_balance_change", # margin balance % change vs 30d ago
    "market_breadth",       # fraction of up-ticking stocks on that day
]

ALL_FEATURE_COLS = GROUP_A_COLS + GROUP_B_COLS + GROUP_C_COLS + GROUP_D_COLS
N_FEATURES = len(ALL_FEATURE_COLS)  # 7+5+19+5 = 36
...
def compute_technical_features(bars_df: pd.DataFrame) -> dict[str, float]:
    ...
    Args:
        bars_df: DataFrame with columns [close, high, low, volume, turnover_rate],
                 sorted ascending by date. Must have at least 2 rows.

    Returns:
        Dict of feature name → float value.
        All 19 Group C fields (GROUP_C_COLS).
    """
...
class FeatureBuilder:
    ...
    def _load_sentiment(self, symbol: str, target_date: date,
        ...
        """Load sentiment factors from Gold layer for a symbol around target_date."""
        import duckdb
    ...
    def _kg_features(self, event_type: str, symbol_sector: str) -> dict[str, float]:
        ...
        Falls back to zeros if knowledge graph is not available.
        """
        try:
            ...
            # Map sector name to SW enum (e.g. "SW_Electronics" → SW.Electronics)
    ...
    def _market_features(self, target_date: date) -> dict[str, float]:
        ...
        # Approximate market trend using 000001.SH (Shanghai Composite)
        benchmark = "000001.SH"
    ...
    def build(self,
        ...
        group_a: dict[str, float] = {
            ...
            "sentiment_score":       float(sent.get("sentiment_score", event.sentiment_score)),
            "event_recency_penalty": 1.0,   # filled below
